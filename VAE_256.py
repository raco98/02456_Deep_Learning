class ConvolutionalVariationalAutoencoder(nn.Module):
    def __init__(self, input_shape:torch.Size, latent_features: int, in_channels: int = 1) -> None:
        ...
        self.conv_encoder = nn.Sequential(
            ...
            nn.Flatten(),
            # Flattened size: 256 channels * 16 * 16 = 65536
            nn.Linear(256 * 16 * 16, latent_features * 2) 
        )

        
        self.conv_decoder = nn.Sequential(
            nn.Linear(latent_features, 256 * 16 * 16), # Upscale linear layer
            nn.Unflatten(dim=1, unflattened_size=(256, 16, 16)), # Reshape
